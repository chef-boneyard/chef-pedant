  ...
  context "POST /environments/:env/cookbook_versions" do
    ...
    context "empty and error cases" do
      ...
      it "returns 412 with an existing cookbook filtered out by environment" do
        ...
        error_hash = {
          "message"=>
          "Unable to satisfy goal constraint #{cookbook_name} due to constraint on foo\n",
          "unsatisfiable_run_list_item"=>["#{cookbook_name}"],
          "non_existent_cookbooks"=>[],
          "most_constrained_cookbooks"=>"(#{cookbook_name} at non-existent version )"
        }
      ...
      it "returns 412 when there is a runlist entry specifying version that doesn't exist" do
        ...
        error_hash = {
          "message" =>
          "Unable to satisfy goal constraint (#{cookbook_name} = #{cookbook_version2}) due to constraint on foo\n",
          "unsatisfiable_run_list_item"=>["(#{cookbook_name} = #{cookbook_version2})"],
          "non_existent_cookbooks"=>[],
          "most_constrained_cookbooks"=>"(#{cookbook_name} = 1.2.3)"
        }
        post(api_url("/environments/#{env}/cookbook_versions"), admin_user,
    ...
    context "dependency error cases (one cookbook)" do
      ...
      it "returns 412 when there is a dep that doesn't exist" do
        ...
        error_hash = {
          ...
          "unsatisfiable_run_list_item"=>["#{cookbook_name}"],
          "non_existent_cookbooks"=>[],
          "most_constrained_cookbooks"=> "(#{not_exist_name} at non-existent version )"
        }
        post(api_url("/environments/#{env}/cookbook_versions"), admin_user,
             :payload => payload) do |response|
          if ruby?
    ...
    context "dependency error cases (two cookbooks)" do
      ...
      it "returns 412 when there is a dep that doesn't have new enough version" do
        ...
        error_hash = {
          "message" =>
          "Unable to satisfy goal constraint #{cookbook_name} due to constraint on #{cookbook_name2}\n",
           "unsatisfiable_run_list_item"=>["#{cookbook_name}"],
           "non_existent_cookbooks"=>[],
      ...
      it "returns 412 when there is an impossible dependency" do
        ...
        error_hash = {
          "message" => 
          "Unable to satisfy goal constraint #{cookbook_name} due to constraint on #{cookbook_name}\n",
          "unsatisfiable_run_list_item"=>["#{cookbook_name}"],
          "non_existent_cookbooks"=>[],
          "most_constrained_cookbooks"=>"(#{cookbook_name} at non-existent version )"
        }
